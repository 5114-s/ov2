#!/usr/bin/env python3
"""
MetricAnything depth estimation for OVM3D-Det with multi-GPU support.

Usage:
    cd /data/ZhaoX/OVM3D-Det/third_party/metric-anything/models/student_depthmap
    python run_metric_anything_mp.py --root /data/ZhaoX/OVM3D-Det --json-dir datasets/Omni3D_pl-1 --gpu 0
    python run_metric_anything_mp.py --root /data/ZhaoX/OVM3D-Det --json-dir datasets/Omni3D_pl-1 --gpu 1
"""
...
def main():
    ...
    for mode in ['train', 'val']:
        ...
        print(f"\n{mode}: GPU {args.gpu} processing {len(images_to_process)} images (index {start_idx} to {end_idx})")
        ...
        for i, image_info in tqdm(images_to_process, desc=f"GPU {args.gpu} {mode}"):
            ...
            try:
                ...
                prediction = model.infer(input_tensor, orig_size=(orig_h, orig_w), f_px=float(image.width))
